...
const HeadEntry = ({ }) => {
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await axios.get('https://leaveflow.runasp.net/api/Teacher/loadDashboard?userId=' + localStorage.getItem('id'))
    ...
    function sendImageForApi(e) {
        ...
        (async () => {
            const res = await axios.post('https://leaveflow.runasp.net/api/Organizations/addOrganizationPic?id=' + localStorage.getItem('id'), data, {
                headers: {
    ...
    function RemoveImage() {
        (async () => {
            const res = await axios.get('https://leaveflow.runasp.net/api/Organizations/removeOrganizationPic?id=' + localStorage.getItem('id'));
            if (res.status == 200) {
                alert(res.data.message)
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
            ...
            >
                ...
                <div className="space-y-3 flex justify-between items-center py-5 flex-col-reverse lg:flex-row">

                    <div className='mt-10'>
                        <div className="flex items-center gap-3">
                            <div className="w-3 h-3 rounded-full bg-teal-500"></div>
                            <p className="text-gray-700 dark:text-gray-300">{userData.head}</p>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    >
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-2xl font-semibold text-teal-900 dark:text-teal-200">{userData.leavesCount}</p>
                            </div>
                ...
                <Link to={'/staff'}>
                    ...
                    >
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-sm text-teal-800/70 dark:text-teal-300/70 mb-1">Organization Staff</p>
                                <p className="text-2xl font-semibold text-teal-900 dark:text-teal-200">{userData.membersCount}</p>
                            </div>
                            <div className="p-2 rounded-lg bg-teal-200/20 dark:bg-teal-800/20">
                                <BsPeople size={22} />
                            </div>
                ...
                <Link to={'/batchsection'}>
                    ...
                    >
                        <div className="flex justify-between items-start">
                            <div>
                                <p className="text-sm text-teal-800/70 dark:text-teal-300/70 mb-1">Batch & Section Manager</p>
                                <p className="text-2xl font-semibold text-teal-900 dark:text-teal-200">{userData.batchCount}</p>
                            </div>
                            <div className="p-2 rounded-lg bg-teal-200/20 dark:bg-teal-800/20">
                                <BsPeople size={22} />
                            </div>

...
const LeaveRequest = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        // if it was asking by the teacher then role is CLASS 
        // if it was asking by the Principal then role is not CLASS else it is ORGHEAD
        
        const res = await axios.get(`https://leaveflow.runasp.net/api/LeaveRequests/GetRequestedLeaves/${localStorage.getItem('role')=="CLASS"?true:false}?toHigherId=${localStorage.getItem('id')}`)//+localStorage.getItem('role')=="ORGHEAD"?true:false+'?toHigherId=' + localStorage.getItem('id'))
        if (res.status == 200) {
          if (res.data.leaves) {
